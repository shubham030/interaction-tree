...
/** Tool definition for the toolbox */
interface ToolDef {
  name: string;
  description: string;
  params: { name: string; type: string; description: string; required?: boolean }[];
  /** The daemon action to call */
  action: string;
}
 ...
 */
function generateScript(tool: ToolDef, daemonUri: string): string {
 ...
 */
export function generateToolbox(daemonUri: string = 'ws://127.0.0.1:9877'): string {
 ...
 */
export function cleanupToolbox(toolboxPath: string): void {
  try {
